...
export default function WalletPage() {
  ...
  const fetchWallets = async () => {
    try {
      const res = await fetch("http://localhost:10000/api/wallets");
      ...
      if (data.success) {
        // ✅ Wallet list
        const safeWallets = (data.wallets || []).map((w: any) => ({
          userId: w.userId,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-4 gap-6">
        ...
        <StatCard
          icon="📊"
          value={`₹${formatINR(stats.averageBalance)}`}
          label="Average Balance"
          accentColor="#1e40af"
        />
        ...
        <StatCard
          icon="💰"
          value={`₹${formatINR(stats.todayBalance)}`}
          label="Today Deposit"
          accentColor="#9333ea"
        />
      </div>
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-[#e5e7eb]">
                ...
                <th className="py-3 px-4 text-left">Last Txn</th>
              </tr>
            </thead>
