...
export default function Navbar() {
  ...
  return (
    ...
    <>
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed inset-0 z-40 bg-black/50 backdrop-blur-sm transition ${mobileOpen ? "opacity-100 visible" : "opacity-0 invisible"
          }`}
        onClick={closeMenu}
      />

      <div
        className={`fixed top-16 left-0 right-0 z-50 bg-[hsl(var(--background))] border-t border-[hsl(var(--border))] transition-all duration-300 ${mobileOpen ? "translate-y-0 opacity-100" : "-translate-y-4 opacity-0 pointer-events-none"
          }`}
      >
